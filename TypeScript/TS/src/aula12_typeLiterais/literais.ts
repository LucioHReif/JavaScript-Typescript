...
Isso significa que "Olá Mundo" é uma string, mas uma string não é um "Olá Mundo" dentro do sistema de tipos.
...
Veja como isso transforma `nome: "Sabrina"` para `nome: string` mesmo que definido como uma constante. Isso acontece pelo fato do nome
poder ser alterado a qualquer momento:    meuUsuario.nome = "Cynthia";

Porque o nome do objeto meuUsuario poder ser alterado, o TypeScript não pode usar a versão literal no sistema de tipagem.
No entanto existe uma funcionalidade que permite você fazer isso.
const meuUsuarioImutavel = {
  nome: "Fatma",
} as const;

Quado "as const" é aplicado ao objeto, ele se torna um objeto literal que não pode ser alterado ao invés de um objeto mutável.
--> "as const" é uma ótima ferramenta para dados fixos e lugares onde você trata código como literal.
--> "as const" também funciona com arrays:
